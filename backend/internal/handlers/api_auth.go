...
type registerResidentReq struct {
	...
	JKID     string `json:"jk_id"` // Optional: selected JK ID
}
...
func (h *Handler) APIRegisterResident(c *gin.Context) {
	...
	u, err := h.svc.RegisterResident(c.Request.Context(), req.FullName, req.Phone, req.Email, req.IIN, req.Password, req.JKID)
	if err != nil {
		c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		return
	}

	// Resolve target JKID: direct assignment -> EgovStub match
	if u.JKID == nil || *u.JKID == "" {
		jks, _ := h.svc.EgovStub(c.Request.Context(), req.IIN)
		if len(jks) == 1 {
			u.JKID = &jks[0].ID
			_ = h.svc.UpdateUserJKID(c.Request.Context(), u.ID, jks[0].ID)
		}
	}
...
func (h *Handler) APILogin(c *gin.Context) {
	...
	// Resolve Admin JK linkage
	if u.HasRole(models.RoleAdmin) && (u.JKID == nil || *u.JKID == "") {
		jks, _ := h.svc.ListJKs(c.Request.Context())
		for _, j := range jks {
			if j.OwnerID == u.ID {
				u.JKID = &j.ID
				_ = h.svc.UpdateUserJKID(c.Request.Context(), u.ID, j.ID)
				break
			}
		}
	}

	// Resolve Resident JK linkage (check assigned u.JKID first, then eGov stub)
	if u.HasRole(models.RoleResident) && (u.JKID == nil || *u.JKID == "") && u.IIN != nil {
		jks, _ := h.svc.EgovStub(c.Request.Context(), *u.IIN)
		if len(jks) == 1 {
			u.JKID = &jks[0].ID
			_ = h.svc.UpdateUserJKID(c.Request.Context(), u.ID, *u.JKID)
		}
	}
